...
export function md5(input: string): string {
  ...
  function toWords(data: Uint8Array): number[] {
    const withLength = new Uint8Array(((data.length + 8) >> 6) + 1 << 6)
    withLength.set(data)
    withLength[data.length] = 0x80
    const bitLength = data.length * 8
    const view = new DataView(withLength.buffer)
    view.setUint32(withLength.length - 8, bitLength, true)
    view.setUint32(withLength.length - 4, Math.floor(bitLength / 0x1_0000_0000), true)

    const words: number[] = []
    for (let i = 0; i < withLength.length; i += 4) {
